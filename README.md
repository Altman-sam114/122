...
v3.6 已起步处理发布级拿战 UI：`NapoleonicDesignTokens` 集中保存拿战面板、描边和状态色；`HUDView`、`UnitInspectorView`、`UnitTooltipView` 和 `EconomyPanelView` 已开始用 `Steady/Shaken/Broken`、`Fresh/Tired/Exhausted`、`Ready/Strained`、`Low/Empty` 等文字加颜色表达士气、疲劳、弹药和预备队状态。HUD 标题、RootGameView accessibility label 和 SpriteKit empty board title 已改读 `ScenarioCatalog.displayName(for:)`，不再硬编码 `Ardennes V0`；HUD 拿战路径的 active faction 指标显示为 Active Power。拿战 faction 下 map layer picker、compact tabs、日志分类、interaction log、`CommandResult.message`、规则事件日志、tooltip/VoiceOver、SpriteKit 单位棋子、增援入口、目标点 marker、`WarDirectiveRecord` recent replay 线和 tactic marker、单位/地域/命令/将军/指挥官档案/外交/AI 复盘面板会显示 Sector / Active Wing / Contact / Corps、Formation / Orders / Corps Order / Order executed/rejected / Hold Line / Square Hold / Withdrawal / Commander Profile / Coalition / Staff / Command Dispatch / Dispatch Audit 等术语；Command panel 会在选中 infantry-heavy formation 时提前提示 square-ready Hold 可削弱骑兵。Region inspector 和 unit strategic summary 的拿战空态也显示为 Uncontrolled / Not present / No active wing assigned / No corps sector assigned / No listed objectives / No formations / No visible enemy formations 等 display-only 面板文案，AI 面板空状态和拿战 provider 显示也不再暴露 Guderian / MockAI 占位。当前规则/指令层已有最小炮兵准备与骑兵冲锋 tactic slice，但仍未完成完整地图美术、发布级炮击/冲锋视觉路径、指挥官头像、完整战报回放面板或截图验收。

HUD、Command panel、Agent panel、Diplomacy panel、commander profile / corps command、region inspector、unit inspector、unit tooltip 和 SpriteKit region / objective 摘要现在也按拿战显示上下文净化可见 formation 名、allegiance 名、目标控制状态、component type code、raw id fallback、contact line 与 composition 空态；这些只改变 display text，不改 `Division`、`Faction`、`ComponentType`、`GameState` 或规则层数据。Waterloo 场景 release-facing displayName 已收口为 `Waterloo 1815`，region 数据显示名已收口为 `Waterloo 1815 Sectors`，数据切片边界继续只写在数据说明 / 文档中。
...
| `UI/AgentPanelView.swift` | 决策展示 | 读 `record`（agent/provider/intent/context/command results/errors/raw JSON）；拿战 Full 层标题显示 Dispatch Audit，底层 rawJSON 保留 |
