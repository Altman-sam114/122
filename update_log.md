...
- `AppContainer.aiNoActionFeedbackMessage` 会在 AI 回合结束后检查 `AgentDecisionRecord.commandResults`，若没有非 End Turn 的已执行战场命令，则向 interaction log 追加 `Staff note` / `AI note`；record-level 诊断会先由 `aiDiagnosticFeedbackMessages` 统一写入可见的 `Staff dispatch issue` / `AI issue`。
...
- 2026-07-06：Waterloo strongpoint / replay wording follow-up。`waterloo_1815_scenario.json` 将 `aa_la_haye_sainte_garrison.templateId` 从 `line_infantry_brigade` 调整为既有 `strongpoint_guard`，HP 仍为 8，使 La Haye Sainte 初始守军与该 tile / keyLocation / objective / region 的 strongpoint / fortress 口径一致；不改 objective、victory condition、reinforcement、schema 或 public API。`GameSaveSnapshot`、`AppContainer`、`RootGameView`、`AgentPanelView` 和 `NapoleonicMessageSanitizer` 继续收口玩家可见工程词：清理/坏存档提示不再显示 snapshot / schema，directive 详情摘要在拿战路径显示 carried out / refused，sanitizer 覆盖小写 `zone directives` / `mock directives`。同步更新 `README.md`、`md/flow/flow.md` 和 v3 总提示词。本轮按人工要求未运行本地测试、构建、lint、parse、jq 或 `git diff --check`；JSON 解析、Swift 编译、UI 行为、玩法平衡和构建需由 GitHub Actions artifact 验收确认。
- 2026-07-06：Waterloo data / staff wording follow-up。`waterloo_1815_scenario.json` 为 q4,r0 Prussian Approach 增加非 objective 的 `key_prussian_approach_screen` road marker，用于标记已有 `pr_blucher_approach_screen` 前卫 screen 所在接近路；不改 objective、victory condition、reinforcement trigger、entryCoord、schema 或 public API。`AppContainer` 与 `NapoleonicMessageSanitizer` 将拿战路径的 AI completion / resolved / issue / guard paused 可见文案统一为 Staff dispatch，并让连续 AI faction 的 record-level 诊断正文按实际 acting faction 做 sanitizer，避免多势力连跑后受当前 active faction 影响。同步更新 `README.md`、`md/flow/flow.md`、`md/flow/flowchart.md` 和 v3 总提示词。本轮按人工要求未运行本地测试、构建、lint、parse、jq 或 `git diff --check`；JSON 解析、Swift 编译、UI 行为和构建需由 GitHub Actions artifact 验收确认。
- 2026-07-06：Waterloo data / sanitizer follow-up。`waterloo_1815_scenario.json` 为 q2,r1 Mont-Saint-Jean 后方补入非 objective 的 `key_anglo_allied_rear_road` road marker，用于标记 Anglo-Allied supply road hex；不改 objective、region objective、victory condition、reinforcement trigger、entryCoord、schema 或 public API。`NapoleonicMessageSanitizer` 继续覆盖 Full 详情和诊断中可能露出的 raw JSON / raw command / pipeline / schemaVersion / snapshot 等工程词，只改展示文本，不改底层 rawJSON、保存格式或命令管线。同步更新 `README.md`、`md/flow/flow.md`、`md/flow/flowchart.md` 和 v3 总提示词。本轮按人工要求未运行本地测试、构建、lint、parse、jq 或 `git diff --check`；JSON 解析、Swift 编译、UI 行为和构建需由 GitHub Actions artifact 验收确认。
- 2026-07-06：MockAI Waterloo fallback sorting follow-up。`MockAIClient` 在 fallback objective move 中按 Waterloo 已知 objective id 和当前 faction 做 deterministic objective-aware sorting：France 优先 Mont-Saint-Jean / La Haye Sainte，Prussia 优先 Prussian Arrival Road / Plancenoit，其他 legacy 或未知 objective 仍回退 kind / name / id 稳定排序。该改动只重排 simulated staff fallback 目标候选，不新增/修改 objective、victory condition、reinforcement、schema 或 public API，不绕过 Command / ZoneDirective -> WarCommandExecutor / CommandValidator / RuleEngine。同步更新 `README.md`、`md/flow/flow.md`、`md/flow/flowchart.md` 和 v3 总提示词。本轮按人工要求未运行本地测试、构建、lint、parse、jq 或 `git diff --check`；Swift 编译、AI fallback 行为和构建需由 GitHub Actions artifact 验收确认。
- 2026-07-06：RegionInspector / unit strategic summary / Waterloo dataNotes wording follow-up。`RegionInspectorView` / `MapDisplayAdapter` 在拿战路径下将若干空值显示从通用 None 收口为 Uncontrolled / Not present / No active wing assigned / No corps sector assigned / No listed objectives / No formations / No visible enemy formations；legacy faction 保持旧显示。`waterloo_1815_scenario.json` 仅补充 dataNotes，说明既有 q2,r1 Anglo-Allied Rear Road 是非 objective 的 Mont-Saint-Jean 后方补给路 marker。不新增 objective、region、unit、reinforcement、victory condition、schema 或规则行为。同步更新 `README.md`、`md/flow/flow.md` 和 v3 总提示词。本轮按人工要求未运行本地测试、构建、lint、parse、jq 或 `git diff --check`；Swift 编译、JSON 解析、UI 行为和构建需由 GitHub Actions artifact / 后续人工运行时验收确认。
- 2026-07-07：Unit inspector / tooltip / Waterloo display wording follow-up。`UnitInspectorView`、`UnitTooltipView` 和 `MapDisplayAdapter` 在拿战显示上下文中净化玩家可见 formation 名、allegiance 名、目标 controller status、contact line / composition 空态和 legacy component type code fallback；`RootGameView` 向 tooltip 传入当前 active faction 作为显示上下文。`waterloo_1815_scenario.json` 将 `Lobau's Plancenoit Guard` 改为 `Lobau's Plancenoit Detachment`，并补充 dataNotes 说明 Plancenoit detachment 与 q4,r0 非 objective Prussian Approach road marker。该改动只影响展示文案、单位显示名和 dataNotes，不改 objective、region、unit stats、template、reinforcement、victory condition、schema、public API 或规则行为。同步更新 `README.md`、`md/flow/flow.md` 和 v3 总提示词；`md/flow/flowchart.md` 未改，因为没有新增执行流程节点。本轮按人工要求未运行本地测试、构建、lint、parse、jq 或 `git diff --check`；Swift 编译、JSON 解析、UI 行为和构建需由 GitHub Actions artifact 验收确认。
- 2026-07-07：Release candidate wording / legacy guard maintenance。并发只读审计后，`NapoleonicMessageSanitizer` 增加 `Anglo-Allied` 保护词并继续覆盖 Guderian / Montgomery / Division / panzerDivision / motorizedInfantry 等默认拿战可见 legacy token；这只改 display text，不改 raw JSON、保存格式或规则执行。`HUDView`、`RegionInspectorView`、`GeneralProfileView` 和 `DiplomacyPanelView` 对胜负、controller、将领 faction、assigned formation、country / bloc 名称等潜在 legacy 混入展示继续套用拿战 sanitizer；`AppContainer` 启动、新局、继续和 unavailable saved campaign 文案不再直接展示 commander catalog JSON 名、raw scenario id 或未经净化的底层错误。`TurnManager.runGermanAITurn` 与 `GameAgent.guderian` / `guderianFallback` 标记为 legacy deprecated，并新增 `runLegacyGermanAITurn`、`legacyGuderian`、`legacyGuderianFallback`，`AppContainer` 在既有 Ardennes guard 后调用显式 legacy helper，避免后续默认 Waterloo 管线误接 Guderian / Germany helper。`waterloo_1815_scenario.json.displayName` 与 `ScenarioCatalog` 对齐为 Waterloo 1815，dataNotes 把潜在可见的 Division-level 改为 formation-level，`ScenarioCatalog.waterloo1815DataSlice.migrationStage` 改为 `v3.8_default_playable_slice`；`md/flow/flowchart.md` 修正 macOS bootstrap 图中过期的“失败时降级 Ardennes legacy”描述；`README.md` 区分本机未跑 Xcode / 模拟器与既有云端 build 记录。未改 `Faction.opponent`、`GamePhase.commandPhase` 或 MapEditor 默认 faction 行为；这些仍是后续兼容迁移风险。
- 2026-07-07：Commander / sector display wording follow-up。在远端 release candidate wording / legacy guard maintenance 基础上，`GeneralCommandPanelView` 和 `RegionInspectorView` 继续净化 corps command 与 sector inspector 里的 formation 名称；`NapoleonicMessageSanitizer` 补充 `ZoneDirective`、`divisionId`、`targetDivisionId` 等审计/异常路径精确替换，降低 Full 详情或诊断文本暴露工程词概率。`waterloo_1815_scenario.json` 将 French / Prussian 抽象编组显示为 `French I Corps Detachment`、`Prussian IV Corps Vanguard`；dataNotes 明确这些只是当前小规模 playable formation，当前 Waterloo runtime victory 仍只读取 Mont-Saint-Jean、Hougoumont 和 Prussian Arrival Road，La Haye Sainte / Papelotte / Plancenoit 仍是地图、region 摘要和 AI 排序目标。该改动只影响展示文本、数据名和 dataNotes，不改 objective、region、unit stats、template、reinforcement、victory condition、schema、public API 或规则行为。同步更新 `README.md`、`md/flow/flow.md` 和 v3 总提示词；`md/flow/flowchart.md` 未改，因为没有新增执行流程节点。本轮按人工要求未运行本地测试、构建、lint、parse、jq 或 `git diff --check`；Swift 编译、JSON 解析、UI 行为和构建需由 GitHub Actions artifact 验收确认。
- 2026-07-07：Waterloo display / audit wording follow-up。并发只读子 Agent 扫描默认 Waterloo UI、AI 日志和数据文档后，`NapoleonicMessageSanitizer` 继续补充 objective / template / region / theater / front-zone / staff id 等 raw key 展示替换；`HUDView`、`CommandPanelView`、`AgentPanelView`、`DiplomacyPanelView`、`GeneralCommandPanelView`、`AppContainer`、`WarDirective` 和 `RulerAgent` 在拿战路径下净化 active power、decode diagnostics 和 raw id fallback，降低 stale legacy state 或缺失 display name 时暴露 Germany / Allies / Panzer / Division / Ardennes 等词的概率；`RulerAgent` / `ZoneCommanderAgent` 的可见 rationale 收口为 Sovereign command / Staff rationale / sector / corps orders。`waterloo_1815_regions.json.displayName` 改为 `Waterloo 1815 Sectors`，`waterloo_1815_scenario.json` 的 dataNotes 改为更像 campaign note 的说明，并把 coalition hold 胜负描述对齐当前运行时 `angloAllied` winner 显示。该改动只影响展示文本、数据说明和文档，不改 objective、region、unit stats、template、reinforcement、victory condition、schema、public API 或规则行为。同步更新 `README.md`、`md/flow/flow.md` 和 v3 总提示词；`md/flow/flowchart.md` 未改，因为没有新增执行流程节点。本轮按人工要求未运行本地测试、构建、lint、parse、jq 或 `git diff --check`；JSON 解析、Swift 编译、UI 行为和构建需由 GitHub Actions artifact 验收确认。
