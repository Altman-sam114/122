...
- `pr_blucher_approach_screen` 是 q4,r0 Prussian Approach 的开局初始屏护单位，和 `ReinforcementState.pending` / `pr_bulow_iv_corps` delayed reinforcement 分开；它不改变 q4,r1 触发 objective、q5,r1 Wavre Road 入口或任何增援时序。Prussian Approach region 保留后方轴线名称，q4,r1 的 key location / region objective 显示名与 scenario objective 统一为 Prussian Arrival Road。
