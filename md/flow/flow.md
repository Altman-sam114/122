...
RootGameView / HUD NewGameButton
  ...
  -> 可选把 activeFaction / phase 切到玩家所选 faction
  -> StrategicStateBootstrapper.bootstrapIfNeeded
  -> GeneralDispatcher.assignGenerals
  -> 清空 selection / highlights / local replay state
```

`NewGameSetupView` 只收集 scenario、玩家 faction 与 Opening Turn 配置；它不直接修改 `GameState`。可选阵营来自所选 scenario JSON 的 `factions`，过滤 neutral 后按 `Faction.turnOrderPriority` 排序。Opening Turn 开启时，`AppContainer` 在新局装配阶段把 `activeFaction` 设为玩家所选 faction，`phase` 设为 `.playerCommand`，清空本回合玩家锁定并重置该 faction 已部署 formation 的 `hasActed`；这不改变 hex controller、division coord、动态战区、部署层或经济账本。若 JSON 读取失败，`AppContainer` 保留当前局面并把失败原因写入 `lastCommandMessage` 与 interaction log；`RootGameView` 会把 `startNewGame` 的返回结果传回 sheet，失败时保留 `NewGameSetupView` 并在本地 Status 区块显示该消息。
...
AI 诊断
  ...
  -> interactionLog 追加 Command dispatch issue / AI issue，最多显示前三条
...
- `ScenarioCatalog.napoleonicTarget.matches(state.scenarioId)` 或存在 Waterloo victory condition id 时，使用最小 Waterloo 节奏：
  ...
  - Plancenoit 当前是地图 objective / region objective / AI 目标排序数据，不属于这两个 runtime victory condition；Wavre Road 当前是 q5,r1 普军后方 road / supply / reinforcement entry hex，不新增 objective，也不属于 Waterloo runtime victory condition。后续若把它们接入胜负节奏，必须同步更新 `waterloo_1815_scenario.json` 的 `victoryConditions` 和本文档。
