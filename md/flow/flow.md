...
- `ScenarioCatalog.napoleonicTarget.matches(state.scenarioId)` 或存在 Waterloo victory condition id 时，使用最小 Waterloo 节奏：
  ...
  - Plancenoit 当前是地图 objective / region objective / AI 目标排序数据，不属于这两个 runtime victory condition；后续若把它接入胜负节奏，必须同步更新 `waterloo_1815_scenario.json` 的 `victoryConditions` 和本文档。
